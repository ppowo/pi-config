...
const MAX_EDIT_TOOL_CACHE_SIZE = 24;
const MAX_HIGHLIGHT_CACHE_ENTRIES = 2048;
const MAX_HIGHLIGHT_LINE_LENGTH = 4000;
const MAX_SPLIT_DIFF_CHARS = 120_000;
...
function getEditTool(cwd: string) {
	...
	if (!tool) {
		if (editToolCache.size >= MAX_EDIT_TOOL_CACHE_SIZE) {
			const oldestKey = editToolCache.keys().next().value;
			if (typeof oldestKey === "string") {
				editToolCache.delete(oldestKey);
			}
		}
...
function sanitizeDisplayText(value: unknown, maxChars: number): string {
	...
	const withoutAnsi = stripAnsi(asString);
	const normalizedBreaks = withoutAnsi.replace(/\r/g, " ").replace(/\n/g, " ");
	const sanitized = sanitizeSingleLineText(normalizedBreaks).replace(/\s+/g, " ").trim();
	return truncateDisplayText(sanitized, maxChars);
}

function countLinesWithinLimit(value: string, limit: number): { lineCount: number; exceeded: boolean } {
	let lineCount = 1;
...
function isDiffTooLargeForSplit(diff: string): { tooLarge: boolean; reason?: string } {
	if (diff.length > MAX_SPLIT_DIFF_CHARS) {
		return {
			tooLarge: true,
			reason: `diff size ${diff.length.toLocaleString()} chars exceeds limit ${MAX_SPLIT_DIFF_CHARS.toLocaleString()}`,
		};
	}

	const lineStats = countLinesWithinLimit(diff, MAX_SPLIT_DIFF_LINES);
	if (lineStats.exceeded) {
		return {
			tooLarge: true,
			reason: `diff has more than ${MAX_SPLIT_DIFF_LINES.toLocaleString()} lines`,
		};
	}
...
class SplitDiffComponent implements Component {
	...
	private syntaxHighlight(line: string): string {
		...
		if (!this.language) return safeLine;
		if (safeLine.length > MAX_HIGHLIGHT_LINE_LENGTH) return safeLine;
		...
		if (this.highlightCache.size >= MAX_HIGHLIGHT_CACHE_ENTRIES) {
...
export default function diffRendererExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		renderCall(args, theme) {
			const safePath = sanitizeDisplayText(args.path, MAX_RENDER_PATH_CHARS) || "<unknown>";
			return new Text(`${theme.fg("toolTitle", theme.bold("edit"))} ${theme.fg("accent", safePath)}`, 0, 0);
		},
